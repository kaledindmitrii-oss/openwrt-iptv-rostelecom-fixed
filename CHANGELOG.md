...
- Исправлен полный restore: после копирования backup больше не выполняется опасный `uci commit`, который мог перезаписать восстановленные файлы незавершёнными staged-изменениями.
- Restore теперь использует временный файл + `mv` для конфигов и проверяет reload основных сервисов.
- IPTV-downstream firewall zone переведена на `input REJECT`; отдельно разрешены только DHCP, DNS и IGMP, чтобы приставка не получала лишний доступ к сервисам управления роутера.
- Усилен preflight: проверяются базовые команды, необходимые установщику.
- Исправлена последовательность Classic: пакет `igmpproxy` не устанавливается до подтверждения пользователя.
- Мастер показывает link state и текущее bridge-членство выбранного порта и предупреждает о выводе порта из bridge.
- Исправлена обработка ошибок project hotplug при rollback/uninstall: резерв не удаляется, если восстановление не удалось.
- Убраны неоднозначные `&& ... || ...` конструкции в критических местах.
- GitHub Actions переведён на системный ShellCheck на фиксированном Ubuntu 24.04; сторонний action-shellcheck удалён.
- Версия проекта: 4.3.3.
